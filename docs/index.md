...
```solidity
function getCurrentContribution(bytes32 tag, address user) external view returns (uint256)
```

Returns the current contribution of a user for a specific tag in the current period

_This function does not apply any weighting to the contribution_
...
Error thrown when already analyzed deposits
...
Error thrown when the recipientSaltHash is already used
...
```solidity
function isDepositOngoing(uint256 depositId, bytes32 recipientSaltHash, uint32 tokenIndex, uint256 amount, address sender) external view returns (bool)
```
...
| Name | Type | Description                      |
| ---- | ---- | -------------------------------- |
| [0]  | bool | if deposit is valid, return true |
